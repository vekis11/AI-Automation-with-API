...
def main():
    ...
    total_tests += 1
    if run_command(
        "python -c \"import requests; import time; start=time.time(); [requests.get('http://localhost:8000/health') for _ in range(100)]; print(f'100 requests in {time.time()-start:.2f}s')\"",
        "Performance Test",
    ):
        tests_passed += 1
